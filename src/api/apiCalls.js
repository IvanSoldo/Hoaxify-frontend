...
export const setAuthorizationHeader = (authentication) => {
  if (authentication.authentication.user.isLoggedIn) {
    axios.defaults.headers.common["Authorization"] = `Basic ${btoa(
      authentication.authentication.user.username +
        ":" +
        authentication.authentication.user.password
    )}`;
  } else {
...
export const editHoax = (hoaxId, body) => {
  return axios.put("/api/1.0/hoaxes/" + hoaxId, body);
};

export const getLikes = (hoaxId) => {
  return axios.get("/api/1.0/hoaxes/" + hoaxId + "/likes");
};

export const handleLike = (hoaxId) => {
  return axios.post("/api/1.0/hoaxes/" + hoaxId + "/likes");
};
